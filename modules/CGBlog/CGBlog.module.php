...
final class CGBlog extends CGExtensions
{
    ...
    public function GetVersion() { return '1.12.1'; }
    ...
    public function GetDependencies() {
        return array('CGExtensions'=>'1.38.5','CGSimpleSmarty'=>'1.7.2','JQueryTools'=>'1.2.6');
    }
...
EOT;
    ...
    public function get_pretty_url($id,$action,$returnid='',$params=array(),$inline=false)
    {
        ...
        case 'details':
            ...
            $article = cgblog_article::get_by_id($articleid,FALSE,FALSE,FALSE);
            if( !is_object($article) ) return;
            if( $article->url ) return $article->url;
            $title = munge_string_to_url($row['cgblog_title']);
            return $this->GetPreference('urlprefix','cgblog')."/{$article->id}/{$article->title}";
            break;
