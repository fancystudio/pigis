...
if (isset($params['detailtemplate'])) $template = 'detail'.$params['detailtemplate'];
...
if( $preview || !$smarty->isCached($this->GetDatabaseResource($template),$cache_id,$compile_id) ) {
  ...
  if( !$article ) {
    if( $articleid == -1 ) {
      $article = cgblog_article::get_latest($canviewdraft);
    }
    else {
      $article = cgblog_article::get_by_id($articleid,$canviewdraft);
    }
  }
  ...
  $smarty->assign('extra_label',$this->Lang('extra'));
}
