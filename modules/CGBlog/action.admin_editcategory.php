...
$_build_editcategory_form = function($category_row = null) use ($params,$smarty)
{
    ...
    $category_list = cgblog_ops::get_category_list(TRUE,$selected_id);
...
try {
    ...
    case 'del':
        ...
        if( $tmp ) throw new CmsException($mod->Lang('error_deleteparent'));

        // get category data.
        $query = 'SELECT * FROM '.cms_db_prefix().'module_cgblog_categories WHERE ID = ?';
        $rec = $db->GetRow($query,array($cid));
        if( !is_array($rec) ) throw new CmsException($mod->Lang('error_notfound'));
